 ...
 * `backtrace()` and `ubacktrace()` returns a string containing a list of addresses in hexademical format
 ...
 * `print_backtrace()` and `print_ubacktrace()` gets stack and prints it immidiately, thus no arguments accepted and no return values supplied. 
...
Printing backtraces involves getting a symbol which matches some memory address which involves digging into `symtab` or similiar section of binary files. Dynamic tracing systems can do that and print (DTrace) or return a symbol as a string (SystemTap) with following functions:
